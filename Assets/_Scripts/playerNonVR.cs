...
public class playerNonVR : MonoBehaviour {
...
#if UNITY_EDITOR
    [CustomEditor(typeof(playerNonVR)),InitializeOnLoadAttribute]
    public class FPAIO_Editor : Editor{
        ...
        #region Camera Setup
            ...
            t.cameraInputMethod = (playerNonVR.CameraInputMethod)EditorGUILayout.EnumPopup(new GUIContent("Input Method", "Determines the method used to rotate camera. \n\nTraditional uses the mouse on all axes. \nTraditional with constraints uses the mouse on the Y axis only. \nRetro uses Keybinds (left and right movement keys) to rotate the camera along the Y axis."),t.cameraInputMethod);
            if(t.cameraInputMethod == playerNonVR.CameraInputMethod.Traditional){t.verticalRotationRange = EditorGUILayout.Slider(new GUIContent("Vertical Rotation Range","Determines how much range does the camera have to move vertically."),t.verticalRotationRange,90,180);}
            if(t.cameraInputMethod == playerNonVR.CameraInputMethod.Traditional || t.cameraInputMethod == playerNonVR.CameraInputMethod.TraditionalWithConstraints){
            t.mouseInputInversion = (playerNonVR.InvertMouseInput)EditorGUILayout.EnumPopup(new GUIContent("Mouse Input Inversion","Determines if mouse input should be inverted, and along which axes"),t.mouseInputInversion);
        ...
        #region Audio/SFX Setup
            ...
            EditorGUILayout.Space();
            t.fsmode = (playerNonVR.FSMode)EditorGUILayout.EnumPopup(new GUIContent("Footstep Mode","Determines the method used to trigger footsetps."),t. fsmode);
            EditorGUILayout.Space();

            #region FS Static
            if(t.fsmode == playerNonVR.FSMode.Static){
                ...
                t.dynamicFootstep.materialMode = (playerNonVR.DynamicFootStep.matMode)EditorGUILayout.EnumPopup(new GUIContent("Material Type", "Determines the type of material will trigger footstep audio."),t.dynamicFootstep.materialMode);
                EditorGUILayout.Space();
                #region Wood Section
                ...
                if(showWoodFS){
                    GUILayout.BeginVertical("box");
                    if(t.dynamicFootstep.materialMode == playerNonVR.DynamicFootStep.matMode.physicMaterial){
